...
impact_method2 <- function(con, disease, dat_cross, dat_life, fvp, meta_stochastic) {
  ...
  if (disease %in% c("HPV", "MenA", "YF", "JE", "Typhoid")) {
  ...
  } else if (disease == "HepB") {
    ## routine age 0
    d1 <- impact_calculate(dat_life, vaccine = "HepB_BD", activity_type = "routine", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
    d2 <- impact_calculate(dat_life, vaccine = "HepB", activity_type = "routine", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
    dat <- merge(d1, d2, by = intersect(names(d1), names(d2)), all = TRUE)
    ##remove double counting
    ## this is a bit mannual, should make it more automated.
    message("the following type1/2 country list varies by touchstone. make sure you always update version and description in the sqls")
    type1_countries <- DBI::dbGetQuery(con, "SELECT DISTINCT country, nid FROM burden_estimate_expectation 
                                      JOIN burden_estimate_country_expectation
                                      ON burden_estimate_country_expectation.burden_estimate_expectation = burden_estimate_expectation.id
                                      JOIN country ON country.id = country
                                      WHERE version = '202110gavi-3' AND description = 'HepB:IC-Hallett:non_bd'")[["nid"]]
    type2_countries <- DBI::dbGetQuery(con, "SELECT DISTINCT country, nid FROM burden_estimate_expectation 
                                      JOIN burden_estimate_country_expectation
                                      ON burden_estimate_country_expectation.burden_estimate_expectation = burden_estimate_expectation.id
                                      JOIN country ON country.id = country
                                      WHERE version = '202110gavi-3' AND description = 'HepB:IC-Hallett:bd'")[["nid"]]
    stopifnot(length(type1_countries) + length(type2_countries) == 112L)
    dat$country_type <- "T1"
    dat$country_type[dat$country %in% type2_countries] <- "T2"
    
    dat <- dat %>%
      mutate_at(names(dat)[grep("type1", names(dat))], funs(replace(., country_type == "T2" & !is.na(.), NA))) %>%
      mutate_at(names(dat)[grep("type2", names(dat))], funs(replace(., country_type == "T1" & !is.na(.), NA)))
    
    d1 <- dat[c("country", "index", "run_id", "year", names(dat)[grepl("type1", names(dat)) | grepl("gavi_support", names(dat))])][dat$country_type == "T1", ]
    names(d1) <- gsub("type1-", "",names(d1))
    d2 <- dat[c("country", "index", "run_id", "year", names(dat)[grepl("type2", names(dat)) | grepl("gavi_support", names(dat))])][dat$country_type == "T2", ]
    names(d2) <- gsub("type2-", "",names(d1))
    dat <- rbind(d1, d2)
  } else if (disease == "Rubella") {
    message("For Rubella, I calculate impact in the same way for both models, where rcv1+rcv2 impact are evaluated as combined.")
    d1 <- impact_calculate(dat_cross, vaccine = disease, activity_type = "campaign", t_min = y_min, t_max = y_max, fvp, meta_stochastic)
    d2 <- impact_calculate(dat_life, vaccine = disease, activity_type = "routine", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
    d3 <- impact_calculate(dat_life, vaccine = "RCV2", activity_type = "routine", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
    dat <- Reduce(function(x,y) merge(x = x, y = y, by = intersect(names(x), names(y)), all = TRUE), 
                  list(d1, d2, d3))     
  } else if (disease %in% c("PCV", "Rota", "Hib")) {
    ## routine age 0
    vaccine <- "Rota"
    vaccine <- ifelse(disease == "PCV", "PCV3", vaccine)
    vaccine <- ifelse(disease == "Hib", "Hib3", vaccine)
    dat <- impact_calculate(dat_life, vaccine = vaccine, activity_type = "routine", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
    
  } else if (disease %in% c("Cholera")) {
    ## routine age 0
    dat <- impact_calculate(dat_cross, vaccine = disease, activity_type = "campaign", t_min = y_min, t_max = max_rout, fvp, meta_stochastic)
